...
class Users(models.Model):
    ...
    user_img = models.FileField(verbose_name='用户头像', upload_to='user_avatar/', max_length=128,
                                default='media/user_avatar/default_avatar.png')
...
class Articles(models.Model):
    ...
    # 如果分类别删除，博文的分类置空
    category = models.ForeignKey(verbose_name='博文分类', to='Category', to_field='id', null=True, blank=True,
    ...
    def article_title_skip(self):
        return format_html('<a href="../../../create/article/{}/mod/">{}</a>'.format(self.pk, self.article_title))

    article_title_skip.short_description = '文章标题'  # 添加函数的描述


class TagSetArticle(models.Model):
    """标签设置博文类"""
    # 如果删除标签则标签置空
    tag = models.ForeignKey(verbose_name='标签名', to='Tag', to_field='id', null=True, blank=True,
                            on_delete=models.SET_NULL)
    # 如果博文被删除，则相应的记录也删除
